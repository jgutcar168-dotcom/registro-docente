...
st.markdown("<p style='text-align: center; font-family: \"Brush Script MT\", cursive; font-size: 25px; color: #5DADE2;'>Maestra Especialista: [Ángela Ortíz Ordóñez]</p>", unsafe_allow_html=True)
st.divider()
    ...
    if evals:
        for ev in evals:
            with st.container(border=True):
                ...
                c1.write(f"📌 **{ev['nombre_alumno']}** | Fecha: {ev['fecha']}")
                if c2.button("🗑️", key=f"del_h_{ev['id']}"):
                    supabase.table("evaluaciones_alumnos").delete().eq("id", ev['id']).execute()
                    st.rerun()