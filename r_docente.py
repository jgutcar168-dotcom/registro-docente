...
class EvaluacionPDF(FPDF):
    ...
    def get_nb_lines(self, w, txt):
        """Calcula líneas aproximadas sin depender de cw de fpdf directamente para evitar errores."""
        if not txt: return 1
        # Estimación segura: ~2.2mm por carácter en Arial 7pt
        # Ancho útil = w - márgenes internos
        ancho_util = w - 2
        caracteres_por_linea = max(ancho_util / 1.8, 1) 
        lineas_manuales = str(txt).count('\n') + 1
        lineas_por_ajuste = len(str(txt)) / caracteres_por_linea
        return max(lineas_manuales, int(lineas_por_ajuste) + 1)
