...
class EvaluacionPDF(FPDF):
    ...
    def header(self):
        ...
        self.set_text_color(255, 110, 64) # Rojo coral bonito
    ...
    def tabla_maestra(self, items):
        ...
        for it in items:
            # CORRECCIÓN: Aseguramos que si un nivel es None, se convierta en texto vacío ""
            textos = [
                str(it.get('letra', '')), 
...
def cabecera_estilizada(titulo):
    st.markdown(f"""
        <div style="background-color: #f0f2f6; padding: 20px; border-radius: 10px; border-left: 8px solid #4f64af; margin-bottom: 20px;">
            <h1 style="margin: 0; color: #1e3d59; font-family: 'Helvetica Neue', sans-serif; font-size: 34px;">{titulo}</h1>
...
if opcion == "👥 Registro de Alumnos":
    ...
    with tab_alu:
      ...
      # --- BUSCA ESTE BLOQUE DENTRO DE TAB_EVAL Y SUSTITÚYELO ---
        ...
        with st.form("f_ev", clear_on_submit=True):
            ...
            for it in res_i:
                ...
                # Función interna para limpiar los "None" de las opciones del radio
                def limpiar_texto(nivel_opcion):
                    if nivel_opcion is None or str(nivel_opcion).lower() == "none" or str(nivel_opcion).strip() == "":
                        return "No definido"
                    return str(nivel_opcion)

                pts[it['letra']] = st.radio(
                    f"Nivel {it['letra']}", 
                    [1, 2, 3, 4], 
                    format_func=lambda x: f"N{x}: {limpiar_texto(it.get(f'nivel_{x}'))}", 
                    key=f"e_{it['letra']}", 
                    horizontal=True
                )
    ...
    with tab_eval:
        if not res_a: st.warning("No hay alumnos.")
            ...
            with st.form("f_ev", clear_on_submit=True):
                ...
                for it in res_i:
                    st.write(f"**{it['letra']} - {it['descripcion']}**")
                    # CORRECCIÓN: Manejo de los niveles para que no salga "None"
                    pts[it['letra']] = st.radio(
                        f"Nivel {it['letra']}", [1,2,3,4], 
                        format_func=lambda x, it=it: f"N{x}: {it.get(f'nivel_{x}') if it.get(f'nivel_{x}') else 'No definido'}", 
    ...
    cabecera_estilizada("📝 Registro de Autoevaluación Docente")
