...
# --- 2. CLASE PDF CON REJILLA CORREGIDA ---
class EvaluacionPDF(FPDF):
    ...
    def tabla_maestra(self, items):
        ...
        for it in items:
            ...
            for i, t in enumerate(textos):
                ...
                self.rect(x_ini, y_ini, w[i], h_fila, style='FD' if i < 2 else 'D')
    ...
    def bloque_alumnos(self, evaluaciones):
        ...
        h_alumno = 7 + (num_items * 6) + 2 # Cabecera + items + margen
        
        # Iteramos de 3 en 3 (por filas)
        for i in range(0, len(evaluaciones), 3):
            # Comprobar si la fila cabe en la página
            ...
            # Dibujar hasta 3 alumnos en la fila actual
            for j in range(3):
                idx = i + j
            ...
            # Al terminar la fila de 3, movemos la Y hacia abajo
            self.set_y(y_inicio_fila + h_alumno + 5)

# --- 3. INTERFAZ (RESTO DEL CÓDIGO SE MANTIENE IGUAL) ---
# ... [Pestañas Alumnos, Ítems, Evaluación y Histórico] ...


# --- 3. INTERFAZ STREAMLIT ---
st.markdown("<h1 style='text-align: center;'>🎓 Registro Aula P.T.: Ángela Ortíz Ordónez</h1>", unsafe_allow_html=True)
