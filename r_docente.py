...
# --- TAB: HISTÓRICO AGRUPADO POR FECHAS ---
with tab_hist:
    ...
    if evals:
        # 2. Agrupar evaluaciones por fecha (YYYY-MM-DD)
        ...
        st.write("### 📄 Generar Informe PDF")
        col_pdf1, col_pdf2 = st.columns([2, 1])
        dia_sel_pdf = col_pdf1.selectbox("Selecciona el día para el informe:", ["Todo el histórico"] + list(dias_disponibles))
        
        if col_pdf2.button("Generar PDF"):
            pdf = EvaluacionPDF()
            pdf.add_page()
            pdf.tabla_maestra(items_ref)
            
            pdf.set_font("Arial", "B", 14)
            pdf.cell(0, 10, f"Resultados: {dia_sel_pdf}", ln=True)
            pdf.ln(5)
            
            # Filtrar evaluaciones según la selección
            if dia_sel_pdf == "Todo el histórico":
                evals_a_imprimir = evals
            else:
                evals_a_imprimir = [e for e in evals if e['fecha'][:10] == dia_sel_pdf]
            
            for ev in evals_a_imprimir:
            ...
            st.download_button(
                ...
                file_name=f"Informe_{dia_sel_pdf.replace('/', '-')}.pdf",
                mime="application/pdf"
            )

        st.divider()

        # 3. Mostrar histórico en la web agrupado por días
        st.write("### 📅 Registros por día")
        for dia in dias_disponibles:
            with st.expander(f"📅 Día: {dia}", expanded=False):
                # Filtrar alumnos de ese día
                ...
                for _, row in alumnos_dia.iterrows():
                    ...
                    with col_info:
                        ...
                        resumen = ", ".join([f"{k}:{v}" for k, v in row['puntos'].items()])
                        st.write(f"👤 **{row['nombre_alumno']}** |  <small>{resumen}</small>", unsafe_allow_html=True)
                    
                    if col_del.button("🗑️", key=f"btn_del_{row['id']}"):
                        supabase.table("evaluaciones_alumnos").delete().eq("id", row['id']).execute()
                        st.success(f"Eliminado registro de {row['nombre_alumno']}")
                        st.rerun()
    else:
