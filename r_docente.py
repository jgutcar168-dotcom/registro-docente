...
if opcion == "👥 Registro de Alumnos":
    ...
    res_a = sorted(
        res_a_raw,
        key=lambda x: (orden_cursos.index(x["curso"]) if x["curso"] in orden_cursos else 999, x["nombre"])
    )
    
    # Cargamos ítems y limpiamos posibles None/"None"/vacíos
    ...
    for item in res_i_raw:
        for n in ['nivel_1', 'nivel_2', 'nivel_3', 'nivel_4']:
            ...
            if val is None or str(val).lower() == "none" or str(val).strip() == "":
                ...
                item[n] = str(val).replace("None", "").strip()
        res_i.append(item)

    with tab_alu:
        c_sel, c_del = st.columns([3, 1])
        sel_a = c_sel.selectbox(
            "Seleccionar Alumno",
            ["+ Nuevo"] + [f"{a['nombre']} ({a['curso']})" for a in res_a]
        )
        v_id, v_nom, v_cur = 0, "", ""
        if sel_a != "+ Nuevo":
        ...
        with st.form("f_alu"):
            ...
            if st.form_submit_button("💾 Guardar Datos"):
                if v_id == 0:
                    supabase.table("alumnos").insert({"nombre": n_in, "curso": cur_in}).execute()
                else:
                    supabase.table("alumnos").update({"nombre": n_in, "curso": cur_in}).eq("id", v_id).execute()
                st.rerun()

    with tab_conf:
        c_sel_i, c_del_i = st.columns([3, 1])
        sel_i = c_sel_i.selectbox(
            "Seleccionar Ítem",
            ["+ Nuevo"] + [f"{i['letra']} - {i['descripcion'][:30]}" for i in res_i]
        )
        v_let, v_des, v_n = "", "", [""] * 4
        if sel_i != "+ Nuevo":
            d = next(i for i in res_i if f"{i['letra']} - {i['descripcion'][:30]}" == sel_i)
            v_let, v_des, v_n = d['letra'], d['descripcion'], [
                d['nivel_1'], d['nivel_2'], d['nivel_3'], d['nivel_4']
            ]
        ...
        with st.form("f_item"):
            ...
            ca, cb = st.columns(2)
            n1 = ca.text_area("N1", v_n[0])
            n2 = cb.text_area("N2", v_n[1])
            n3 = ca.text_area("N3", v_n[2])
            n4 = cb.text_area("N4", v_n[3])
            if st.form_submit_button("💾 Guardar Ítem"):
                supabase.table("configuracion_items").upsert({
                    "letra": l_in,
                    "descripcion": d_in,
                    "nivel_1": n1,
                    "nivel_2": n2,
    ...
    with tab_eval:
        if "reset_eval" not in st.session_state:
            st.session_state.reset_eval = False
        ...
        if not res_a:
            ...
            if st.session_state.get("last_fecha") != fe_ev:
                st.session_state.reset_eval = True
            st.session_state.last_fecha = fe_ev

            evals_h = supabase.table("evaluaciones_alumnos").select("nombre_alumno").eq(
            ...
            # Detectar cambio de alumno
            if st.session_state.get("last_alumno") != al_sel:
                st.session_state.reset_eval = True
            st.session_state.last_alumno = al_sel

            is_done = al_sel.startswith("✅")

            # Resetear radios antiguos si ha cambiado alumno o fecha
            if st.session_state.reset_eval:
                for it in res_i:
                    key = f"e_{it['letra']}"
    ...
    with tab_hist:
        ...
        if evals:
            ...
            if c_pdf.button("🖨️ PDF Informe", use_container_width=True):
                ...
                pdf.add_page()
                ev_f = evals if sel_d == "Ver todos" else [
                    e for e in evals if e['fecha'][:10] == sel_d
                ]
                ...
                st.download_button("⬇️ Descargar", pdf_bytes, f"Informe_{sel_d}.pdf")
            for d in dias:
    ...
    with tab1:
        # 1. CONTROL DE MODO EDICIÓN
        es_edicion = st.session_state.edit_id is not None
        if es_edicion:
            st.warning(f"⚠️ Editando registro SDA {st.session_state.datos_edicion.get('sda')}")
        ...
        # 2. PREPARACIÓN DE VALORES (Si es edición usa el dato, si no, valores por defecto)
        ...
        # Usamos un prefijo en la key para forzar el refresco de los widgets al cambiar de modo
        prefix = "edit" if es_edicion else "new"

        c1, c2 = st.columns(2)
        f_s = c1.date_input("Fecha de Registro", value=f_v, key=f"{prefix}_fecha")
        s_s = c2.number_input("SDA", 1, 20, value=s_v, key=f"{prefix}_sda")

        eval_ae = []
        st.write("### Ítems de evaluación")
        
        for i in it_ae:
            st.write(f"**{i['nombre']}**")
            ca, cb = st.columns([1,4])
            
            # Buscamos el valor guardado para este ítem específico
            valor_guardado = it_e_v.get(i['nombre'], {}).get('valor', "No")
            idx_radio = 0 if valor_guardado == "Sí" else 1
            obs_guardada = it_e_v.get(i['nombre'], {}).get('obs', "")

            # Importante: keys dinámicas para que Streamlit detecte el cambio de datos
            v = ca.radio("Cumplido", ["Sí", "No"], index=idx_radio, key=f"{prefix}_rae_{i['id']}", horizontal=True)
            o = cb.text_input("Observaciones", value=obs_guardada, key=f"{prefix}_oae_{i['id']}")
            
            eval_ae.append({"nombre": i['nombre'], "valor": v, "obs": o})

        st.divider()
        st.write("### Reflexión Final")
        ref1 = st.text_area("Lo que ha funcionado", value=r_v.get('funciona', ""), key=f"{prefix}_ref1")
        ref2 = st.text_area("Dificultades", value=r_v.get('dificultades', ""), key=f"{prefix}_ref2")
        ref3 = st.text_area("Propuestas de mejora", value=r_v.get('mejoras', ""), key=f"{prefix}_ref3")

        if st.button("💾 Guardar Cambios" if es_edicion else "💾 Guardar Autoevaluación", type="primary"):
            payload = {
                "fecha": f_s.isoformat(), 
                "sda": s_s, 
                "items_evaluados": eval_ae, 
                "reflexion_final": {"funciona": ref1, "dificultades": ref2, "mejoras": ref3}
            }
            if es_edicion:
    ...
    with tab2:
        ...
        for r in regs:
            with st.expander(f"📅 {r['fecha']} - SDA {r['sda']}"):
                ...
                # --- REEMPLAZA ESTE BLOQUE EN tab_hist ---
                if c_pdf.button("🖨️ PDF Informe", use_container_width=True):
                    pdf = EvaluacionPDF()
                    pdf.alias_nb_pages() # Necesario para que {nb} funcione
                    pdf.tabla_maestra(res_i)
                    ...
                    # MÉTODO SEGURO PARA OBTENER BYTES
                    pdf_output = pdf.output() 
    
                    # Si devuelve bytes directamente (común en fpdf2)
                    if isinstance(pdf_output, bytes):
                        pdf_bytes = pdf_output
                    else:
                        # Si devuelve string, convertir a bytes
                        pdf_bytes = pdf_output.encode("latin1")

                    st.download_button(
                        label="⬇️ Descargar Informe",
                        data=pdf_bytes,
                        file_name=f"Informe_{sel_d}.pdf",
                        mime="application/pdf"
                    )
                if c2.button("✏️ Editar", key=f"eae_{r['id']}"):
                    st.session_state.edit_id = r['id']
                    st.session_state.datos_edicion = r
                    st.rerun()
                
                if c3.button("🗑️ Borrar", key=f"bae_{r['id']}"):
                    supabase.table("autoevaluaciones").delete().eq("id", r['id']).execute()
                    st.rerun()