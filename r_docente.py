...
class EvaluacionPDF(FPDF):
    ...
    def bloque_alumnos(self, evaluaciones):
        ...
        for i in range(0, len(evaluaciones), 3):
            ...
            if y_inicio_fila > 250: self.add_page(); y_inicio_fila = self.get_y()
            for j in range(3):
                idx = i + j
    ...
    with tab_hist:
        ...
        if evals:
            ...
            if c_pdf.button("🖨️ Generar PDF Informe"):
                ...
                if not isinstance(out_bytes, bytes): out_bytes = out_bytes.encode('latin-1')
        ...
        for r in regs:
            with st.expander(f"📅 {r['fecha']} - SDA {r['sda']}"):
                ...
                if c1.button("🖨️ PDF", key=f"pdf_ae_{r['id']}"):
                    ...
                    if not isinstance(out_ae, bytes): out_ae = out_ae.encode('latin-1')
