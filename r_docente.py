        ...
        if not res_a: st.warning("No hay alumnos.")
            ...
            with st.form("f_ev", clear_on_submit=True):
                ...
                for it in res_i:
                    ...
                    # Busca este bloque y asegúrate de que quede así:
                    pts[it['letra']] = st.radio(
                        f"Nivel {it['letra']}", 
                        [1, 2, 3, 4], 
                        format_func=lambda x: f"N{x}: {it.get(f'nivel_{x}') if it.get(f'nivel_{x}') not in [None, 'None', 'none', ''] else 'No definido'}", 
                        key=f"e_{it['letra']}", 
                        horizontal=True
)
