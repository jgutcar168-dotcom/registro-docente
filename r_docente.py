...
class EvaluacionPDF(FPDF):
    ...
    def get_nb_lines(self, w, txt):
        ...
        self.set_font("Arial", "", 6)  # ↓ bajamos tamaño
        max_width = w - 2
    ...
    def tabla_maestra(self, items):
        ...
        for it in items:
            ...
            # Dibujo correcto de celdas
            x_actual = x_ini
            for i, t in enumerate(textos):
                self.rect(x_actual, y_ini, w[i], h_fila)
                self.set_xy(x_actual, y_ini)
...
with tab_alu:
    ...
    # Ordenar por curso según orden pedagógico y luego por nombre
    orden_cursos = ["INF 3", "INF 4", "INF 5", "INF 5", "1º", "2º", "3º", "4º", "5º", "6º"]

    res_a = sorted(
        res_a,
        key=lambda x: (
            orden_cursos.index(x["curso"]) if x["curso"] in orden_cursos else 999,
            x["nombre"]
        )
    )
...
with tab_eval:
    if not res_a:
        ...
        with st.form("f_ev"):
            ...
            # Construir lista con separadores por curso
            lista_alumnos = []
            curso_actual = None
            for a in res_a:
                if a["curso"] != curso_actual:
                    curso_actual = a["curso"]
                    lista_alumnos.append(f"--- {curso_actual} ---")  # separador visual
                lista_alumnos.append(f"{a['nombre']} ({a['curso']})")
            ...
            if al_ev.startswith("---"):
                st.warning("Selecciona un alumno, no un separador")
                st.stop()

            # Color por etapa/curso
            colores_curso = {
                "INF": "#FFF4CC",   # Infantil
                "1º": "#E6F7FF",
                "2º": "#E6F7FF",
            ...
            curso_sel = al_ev.split("(")[-1].replace(")", "").strip()
            etapa = "INF" if "INF" in curso_sel else curso_sel
            color = colores_curso.get(etapa, "#FFFFFF")

            c1.markdown(
                f"<div style='background-color:{color}; padding:8px; border-radius:5px;'>"
                f"<b>Curso seleccionado:</b> {curso_sel}"
                "</div>",
